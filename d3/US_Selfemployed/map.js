...
d3.queue()
  ...
  .defer(d3.csv, "selfemployed.csv", function(d) {
    selfemployed.set(d.id, +d.pct_selfemployed);
  })
  .await(ready);
...
function ready(error, us, s) {
  ...
  g.append("text")
    ...
    .text("Self-employed - 2016 ACS 5-Year Estimates");
  ...
  svg.append("g")
    ...
    .enter().append("path")
    .attr("fill", function(d) {
      return color(d.pct_selfemployed = selfemployed.get(d.id));
    })
    .attr("d", path)
    .append("title")
    .text(function(d) {
